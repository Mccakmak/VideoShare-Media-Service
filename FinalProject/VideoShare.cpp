...
void VideoShare::addFriendship(const string & userName1, const string & userName2) {
	...
	if (checkUser(userName1) && checkUser(userName2))
	{
		...
		else cout << "Suspended user can not remove friendship!" << endl << endl;

		if (user2->getStatus() == ACTIVE)
		{
	...
	if (checkUser(userName))
	{
		...
		while (temp != NULL)
		{
			...
			while (temp2 != NULL)
			{
				...
				temp2 = temp2->getNext();					// next video
			}
			temp = temp->getNext();							// next friend
		}
	...
	if (checkUser(userName1) && checkUser(userName2))
	{
		...
		while (temp1 != NULL)
		{
			...
			while (temp2 != NULL)
			{
				...
				temp2 = temp2->getNext();	//next user2 friend
			}
			temp1 = temp1->getNext();		//next user1 friend
		}
